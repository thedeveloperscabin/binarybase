...
function MagneticLink({ to, children, onClick }) {
  ...
  return (
    <motion.div
      ref={ref}
      className="relative px-1 py-2 text-black dark:text-white transition-colors group cursor-pointer"
...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { to: "/#work", label: "Work", section: "work" },
    { to: "/#contact", label: "Contact", section: "contact" },
  ];

  const handleNavClick = (to, section) => {
    setMenuOpen(false);

    // If we're on home page, scroll to section directly
      ...
      <div className="max-w-6xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            ...
            >
              <span className={activeSection === link.section ? "gradient-text font-medium" : ""}>
                {link.label}
              </span>
            </MagneticLink>
          ))}
